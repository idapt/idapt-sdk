...
import { type HttpContext, requestRaw } from "../http.js";
...
export class RealtimeApi {
  ...
  broadcast(
    ...
    message: unknown,
    opts: CallOptions = {},
  ): Promise<V1Result<"realtime broadcast">> {
    return executeCommand<V1Result<"realtime broadcast">>(
      COMMAND_BINDINGS["realtime broadcast"],
      { channel, message },
      this.ctx,
      opts,
    );
  ...
  ): Promise<V1Result<"realtime presence">> {
    return executeCommand<V1Result<"realtime presence">>(
      COMMAND_BINDINGS["realtime presence"],
      { channel, meta: opts.meta, ttl_seconds: opts.ttlSeconds },
      this.ctx,
      { signal: opts.signal },
    );
  }
  ...
  async presence(
    channel: string,
    opts: CallOptions = {},
  ): Promise<V1Result<"realtime presence-list">["data"]> {
    const res = await executeCommand<V1Result<"realtime presence-list">>(
      COMMAND_BINDINGS["realtime presence-list"],
      { channel },
      this.ctx,
      opts,
    );
