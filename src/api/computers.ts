...
export interface ComputerExposure {
  id: string;
  computer_id: string;
  workspace_id: string;
...
export interface ExposeComputerInput {
...
export class ComputersApi {
  ...
  async fsList(
    ...
    const result = (await this.fs(id, { op: "list", path }, opts)) as {
      path: string;
      entries: FsEntry[];
    };
    return result;
  ...
  async expose(
    id: string,
    input: ExposeComputerInput,
    opts: CallOptions = {},
  ): Promise<ComputerExposure> {
    const res = await request<SingleEnvelope<{ exposure: ComputerExposure }>>(
      this.ctx,
      {
        method: "POST",
  ...
  async unexpose(
    ...
    workspaceId: string,
    opts: CallOptions = {},
  ): Promise<{ unexposed: boolean }> {
    const res = await request<SingleEnvelope<{ unexposed: boolean }>>(
      this.ctx,
      {
        method: "DELETE",
        path: `/api/v1/computers/${id}/expose/${workspaceId}`,
  ...
  async transfer(
    ...
    const res = await request<SingleEnvelope<{ computer: Computer }>>(
      this.ctx,
      {
        method: "POST",
