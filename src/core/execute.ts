  ...
  for (const param of binding.pathParams) {
    ...
    if (value === undefined || value === null || value === "") {
      ...
      const example = Object.fromEntries(
        binding.pathParams.map((p) => [p, "<name or id>"]),
      );
      throw new IdaptError({
        code: "invalid_request",
        message:
          `missing required path argument \`${param}\` for \`${binding.command}\` ` +
          `(resource reference — pass a path, name, or id; resolved server-side). ` +
          `Required path arguments: ${binding.pathParams.join(", ")}. ` +
...
export async function awaitOperation<T = unknown>(
  ...
  const maxAttempts =
    opts.maxPollAttempts ?? binding.pollHint?.maxAttempts ?? 120;
