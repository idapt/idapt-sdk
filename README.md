...
Typed JavaScript / TypeScript client for the [idapt](https://idapt.app) v1
public API. Same surface as the [`idapt` CLI](https://idapt.app/cli) —
workspaces, agents, chats, files, computers, automations, functions, sharing,
...
1. **Browser-apps** — your code runs on a `*.idapt.host` subdomain. The
...
| **Computers** | `computers.exec / tmux / sftp{,Upload,Download} / lifecycle / ports / users / env / pair` |
| **Automations** | `automations.create / fire (webhook-secret) / rotateSecret / listRuns` |
| **Functions** | `functions.create / deploy / invoke / runs.list / runs.get / runs.interrupt` |
...
`@idapt/sdk/react` — see the [main docs](https://idapt.app/developers).
